...
import { Shirt, Plus, X, Check, Trash2, Edit3, ChevronDown, Tags, Type, AlignLeft, Briefcase, CheckCircle2, Package, Eye, ArrowRight, Download } from 'lucide-react-native';
...
export default function Wardrobe({ userId, tripId, isSettingsMode, onClose }: WardrobeProps) {
  ...
  // Selection Logic (Using Array for better React state tracking)
  ...
  const fetchWardrobe = async () => {
    ...
    const { data: wardrobeData } = await supabase.from('wardrobe').select('*').order('created_at', { ascending: false });
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        <View>
          <Text style={[styles.headerSubtitle, { color: theme.tabIconDefault }]}>COLLECTION</Text>
      ...
      {/* 📥 SIDE DOCK (Trip Rack) */}
      <AnimatePresence>
        {tripId && selectedIds.length > 0 && (
          <MotiView 
            from={{ translateX: 100, opacity: 0 }} 
            animate={{ translateX: 0, opacity: 1 }} 
            exit={{ translateX: 100, opacity: 0 }}
            style={styles.sideDock}
          >
            <TouchableOpacity style={[styles.dockContent, { backgroundColor: theme.tint }]} onPress={handleHangSelected}>
              <Image source={HANGER_ICON} style={styles.dockIcon} />
              <Text style={styles.dockText}>{selectedIds.length}</Text>
              <Text style={styles.dockLabel}>HANG</Text>
            </TouchableOpacity>
          </MotiView>
        )}
      </AnimatePresence>
...
const styles = StyleSheet.create({
  ...
  sideDock: { position: 'absolute', right: 0, top: '35%', zIndex: 1000 },
  dockContent: { width: 70, height: 120, borderTopLeftRadius: 35, borderBottomLeftRadius: 35, justifyContent: 'center', alignItems: 'center', elevation: 15, shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 10 },
  dockIcon: { width: 30, height: 30, tintColor: 'white', resizeMode: 'contain' },
  dockText: { color: 'white', fontSize: 18, fontWeight: '900', marginTop: 5 },
  dockLabel: { color: 'white', fontSize: 10, fontWeight: '900', letterSpacing: 1 },
