...
import { View, StyleSheet, TouchableOpacity, ScrollView, TextInput, Modal, Alert, Dimensions, Image, Pressable, Platform, ActivityIndicator, DeviceEventEmitter, KeyboardAvoidingView, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
import { ChevronLeft, Plus, X, Brain, CheckCircle2, AlertCircle, Sparkles, Image as ImageIcon, Flame, History, Trophy, Rotate3d, Check, Ghost, Palette, Trash2, Repeat, FastForward, Search, Pencil } from 'lucide-react-native';
...
export default function DeckDetailScreen() {
  ...
  const saveCard = async () => {
    ...
    const payload = { 
      ...
      difficulty: editingCard?.difficulty || null, 
      next_review: editingCard?.next_review || new Date().toISOString(), 
      interval_days: editingCard?.interval_days || 0, 
      ease_factor: editingCard?.ease_factor || 2.5, 
      review_count: editingCard?.review_count || 0, 
      created_at: editingCard?.created_at || now 
    };
    try {
      if (editingCard) {
        db.runSync(`UPDATE study_cards SET front_content = ?, back_content = ?, front_image_url = ?, back_image_url = ?, options = ?, custom_color = ? WHERE id = ?`, 
          [payload.front_content, payload.back_content, payload.front_image_url, payload.back_image_url, JSON.stringify(payload.options), payload.custom_color, cardId]);
      ...
      setFront(''); setBack(''); setFrontImg(null); setBackImg(null); setOptions([]); setCustomColor(null); setEditingCard(null);
  ...
  const deleteCard = (cardId: string) => {
    Alert.alert("Delete Card?", "This action cannot be undone.", [
      ...
      { text: "Delete", style: 'destructive', onPress: () => {
        ...
        refreshFromSQLite(); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
      }}
    ]);
  };
        ...
        <View style={styles.header}>
          <View style={styles.headerTop}>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <TouchableOpacity onPress={() => { setIsSearchVisible(true); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }} style={[styles.headerBtn, { backgroundColor: theme.card }]}><Search size={20} color={theme.text} /></TouchableOpacity>
              {!isReviewing && <TouchableOpacity onPress={() => { setEditingCard(null); setFront(''); setBack(''); setFrontImg(null); setBackImg(null); setOptions([]); setIsAddModalVisible(true); }} style={[styles.addBtn, { backgroundColor: theme.tint }]}><Plus size={24} color="white" /></TouchableOpacity>}
            </View>
          </View>
          ...
          <AnimatePresence>
            {isSearchVisible && (
              <MotiView from={{ opacity: 0, height: 0, marginTop: 0 }} animate={{ opacity: 1, height: 46, marginTop: 10 }} exit={{ opacity: 0, height: 0, marginTop: 0 }} style={[styles.searchBar, { backgroundColor: theme.card }]}>
                ...
                <TextInput style={[styles.searchInput, { color: theme.text }]} placeholder="Search cards..." placeholderTextColor={theme.tabIconDefault} value={searchQuery} onChangeText={setSearchQuery} autoFocus />
                <TouchableOpacity onPress={() => { setIsSearchVisible(false); setSearchQuery(''); }}><X size={18} color={theme.text} /></TouchableOpacity>
              </MotiView>
            )}
          </AnimatePresence>
        </View>
        ...
        {!isReviewing ? (
          <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            {filteredCards.map((card) => (
              <TouchableOpacity key={card.id} style={[styles.cardItem, { backgroundColor: theme.card }]} onPress={() => { setEditingCard(card); setFront(card.front_content); setBack(card.back_content); setFrontImg(card.front_image_url); setBackImg(card.back_image_url); setOptions(card.options || []); setCustomColor(card.custom_color); setIsAddModalVisible(true); }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={[styles.cardNext, { color: theme.tabIconDefault }]}>{card.difficulty?.toUpperCase() || 'NEW'} • {format(new Date(card.next_review), 'MMM d')}</Text>
                </View>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                  <TouchableOpacity onPress={() => deleteCard(card.id)}><Trash2 size={16} color="#FF3B30" opacity={0.5} /></TouchableOpacity>
        ...
        <Modal visible={isAddModalVisible} transparent animationType="fade">
          <View style={styles.modalOverlay}>
            ...
            <View style={[styles.modalContent, { backgroundColor: theme.card }]}>
              <View style={styles.modalHeader}><Text style={[styles.modalTitle, { color: theme.text }]}>{editingCard ? 'Edit' : 'Add'} Cue Card</Text><TouchableOpacity onPress={() => setIsAddModalVisible(false)} style={styles.modalClose}><X size={20} color={theme.text} /></TouchableOpacity></View>
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 20 }}>
                ...
                <TouchableOpacity onPress={saveCard} style={[styles.saveBtn, { backgroundColor: theme.tint }]}><Text style={styles.saveBtnText}>{editingCard ? 'Update' : 'Save'} to Deck</Text></TouchableOpacity>
              </ScrollView>
            </View>
          </View>
        </Modal>
...
const styles = StyleSheet.create({
  ...
  headerBtn: { width: 44, height: 44, borderRadius: 15, justifyContent: 'center', alignItems: 'center', marginLeft: 8 },
  ...
  searchBar: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, borderRadius: 14, overflow: 'hidden', gap: 10 },
  ...
  listHeader: { marginBottom: 15, marginTop: 10 },
