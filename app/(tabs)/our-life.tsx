...
import { StyleSheet, View, Pressable, TouchableOpacity, Text, Dimensions, Alert, Image, ActivityIndicator, DeviceEventEmitter, Modal } from 'react-native';
...
export default function OurLifeScreen() {
  ...
  useEffect(() => {
    // Hierarchical navigation listener for tab press
    const sub = DeviceEventEmitter.addListener('our-life-tab-press', () => {
      handleOurLifePress();
    });
    return () => sub.remove();
  }, [activeTripId, isPlanMode, location, pins]);
  ...
  useEffect(() => {
    (async () => {
      ...
      // Initial fetch for centering
      const { data } = await supabase.from('places').select('*').is('trip_id', null);
      if (data) {
        setPins(data);
        if (data.length > 0 && mapRef.current && !activeTripId && !isPlanMode) {
          const coords = data.map(p => ({ latitude: p.latitude, longitude: p.longitude }));
          mapRef.current.fitToCoordinates(coords, {
            edgePadding: { top: 100, right: 100, bottom: 100, left: 100 },
            animated: true
          });
        } else if (mapRef.current && !activeTripId && !isPlanMode) {
  ...
  }, [activeTripId, isPlanMode]);

  const fetchPins = async () => {
    let query = supabase.from('places').select('*');
    if (activeTripId) {
      query = query.eq('trip_id', activeTripId);
    } else {
  ...
  if (isPlanMode && !activeTripId) {
    return (
      <View style={{ flex: 1 }}>
        <PlansListScreen onSelectTrip={(id) => setActiveTripId(id)} onClose={() => setIsPlanMode(false)} />
      </View>
    );
  }
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View style={styles.container}>
        {activeTripId && (
          <TripWorkspace 
            tripId={activeTripId} userId={userId} mapRef={mapRef} 
            onBack={() => setActiveTripId(null)} onMarkersChange={(m) => setWorkspaceMarkers(m)} 
        ...
        <MapView
          ...
          style={styles.map}
          initialRegion={{ latitude: location?.coords.latitude ?? 37.78825, longitude: location?.coords.longitude ?? -122.4324, latitudeDelta: 0.0922, longitudeDelta: 0.0421 }}
          ...
          {activeTripId ? (
            ...
            pins.map(pin => (
              <Marker
                ...
                onPress={(e) => { e.stopPropagation(); setActivePin(pin); }}
                anchor={{ x: 0.5, y: 1 }}
              >
                ...
                {isMenuOpen && (
                  <MotiView style={styles.fabSubMenu}>
                    <MotiView from={{ opacity: 0, translateY: 20 }} animate={{ opacity: 1, translateY: 0 }} exit={{ opacity: 0, translateY: 20 }}>
                      <TouchableOpacity style={[styles.subActionButton, { backgroundColor: theme.tint }]} onPress={() => { setSearchVisible(true); setIsMenuOpen(false); }}><Search size={22} color="white" /></TouchableOpacity>
                    </MotiView>
                    <MotiView from={{ opacity: 0, translateY: 20 }} animate={{ opacity: 1, translateY: 0 }} exit={{ opacity: 0, translateY: 20 }}>
                      <TouchableOpacity style={[styles.subActionButton, { backgroundColor: '#555' }]} onPress={() => setIsPlanMode(true)}><Plane size={22} color="white" /></TouchableOpacity>
                    </MotiView>
                  </MotiView>
                )}
              </AnimatePresence>
        ...
        <AnimatePresence>
          {activePin && !activePin.isWorkspaceItem && (
            <PolaroidPopup pin={activePin} onClose={closeActivePin} onEdit={(p) => { setEditingPin(p); setIsAddModalVisible(true); }} isPlanMode={!!activeTripId} />
          )}
        </AnimatePresence>
        ...
        <AddPinModal 
          ...
          onSuccess={() => { fetchPins(); setIsAddModalVisible(false); }} 
          isPlanMode={!!activeTripId} 
...
const styles = StyleSheet.create({
  ...
  subActionButton: { width: 48, height: 48, borderRadius: 24, justifyContent: 'center', alignItems: 'center', elevation: 5 },
  ...
  searchModal: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'white', zIndex: 5000 },
  searchHeader: { height: 100, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#eee' },
  searchTitle: { fontSize: 18, fontWeight: 'bold' },
  webLoader: { ...StyleSheet.absoluteFillObject, backgroundColor: 'white', justifyContent: 'center', alignItems: 'center' },
});
